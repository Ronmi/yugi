...
func init() { addFactory(usrAppointmentDetailTest) }
func usrAppointmentDetailTest() (ret []tmplTestCase) {
	...
	ret = addCase(ret, "usr_appointment_detail.html", usrAppointmentDetailPage{
		Me:   vol,
		Data: app,
	})
	ret = addCase(ret, "usr_appointment_detail.html", usrAppointmentDetailPage{
...
type usrAppointmentDetailPage struct {
	Me   *actions.User
	Data *actions.Appointment
}

func (p *usrAppointmentDetailPage) Render(g *gin.Context) {
	g.HTML(200, "usr_appointment_detail.html", p)
}

func (p *usrAppointmentDetailPage) RenderAPI(g *gin.Context) {
	g.JSON(200, apiData(p.Data))
}

func (srv) UsrAppointmentDetail(c Context) tmplspec {
	type params struct {
	...
	data, err := actions.UsrGetAppointment(c.DB(), me, p.ID)
	if err != nil {
	...
	return &usrAppointmentDetailPage{
		Me:   me,
		Data: data,
	}
}
